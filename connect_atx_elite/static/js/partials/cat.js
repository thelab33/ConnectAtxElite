...
function updateCountdown() {
  ...
  if (diff < 0) diff = 0;
  const days = Math.floor(diff/864e5), hrs = Math.floor(diff/36e5)%24, mins = Math.floor(diff/6e4)%60;
  document.getElementById('cd-days').textContent = days.toString().padStart(2, '0');
  document.getElementById('cd-hours').textContent = hrs.toString().padStart(2, '0');
...
if (elSupporters) {
  ...
  setInterval(() => {
    supporters++;
    elSupporters.textContent = supporters;
  }, Math.floor(13000 + Math.random()*17000)); // simulate new supporter every ~15s
}
...
customAmt && customAmt.addEventListener('focus',()=>{
  presetBtns.forEach(b=>b.classList.remove('selected'));
  selectedAmt = null;
});
...
function launchConfetti() {
  ...
  const pieces = Array.from({length:96},()=>({
    x:Math.random()*W, y:Math.random()*-H, s:Math.random()*8+5, a:Math.random()*2*Math.PI, c:`hsl(${Math.random()*60+35},96%,55%)`
  }));
  let t = 0, frame;
  function draw() {
    ...
    if (t > 100) { cancelAnimationFrame(frame); canvas.style.display='none'; }
  }
  draw();
}
