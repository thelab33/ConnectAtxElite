...
from app import socketio  # Absolute import required
...
DOMAIN = os.getenv("BASE_URL", "http://localhost:8000").rstrip("/")
DEFAULT_DONATION = 25  # USD
MIN_DONATION = 1
MAX_DONATION = 999
STATIC_DONATION_LINK = os.getenv("DONATION_LINK")

# ─── Utilities ────────────────────────────────────────────────────────────
_AMOUNT_RE = re.compile(r"^elite\s+(\d+)", re.I)

def _validate_amount(dollars: int) -> int:
    """Clamp and return a safe donation amount in dollars."""
    return max(MIN_DONATION, min(MAX_DONATION, dollars))

def _error(message: str, code: int = 400) -> tuple[dict[str, str], int]:
    """Helper to return a JSON API error."""
...
def sms_webhook() -> Response:
    ...
    Expected body example: Body="ELITE 50"
    """
