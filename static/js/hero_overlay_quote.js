const QUOTES = [
  ...
  },
  {
    ...
    avatar: "/static/coach_lay.jpg",
  },
];

let idx = 0;
const card = document.getElementById('hero-quote-card');
const interval = 6700;
let timer = null;
...
document.getElementById('hero-prev-quote').addEventListener('click', showPrevQuote);
document.getElementById('hero-next-quote').addEventListener('click', showNextQuote);
document.getElementById('hero-copy-quote').addEventListener('click', () => {
  const q = QUOTES[idx];
...
document.getElementById('hero-share-quote').addEventListener('click', () => {
  const q = QUOTES[idx];
  const text = `“${q.text}” — ${q.author}`;
  if (navigator.share) {
    navigator.share({ title: 'Connect ATX Elite', text }).catch(() => {});
  } else {
